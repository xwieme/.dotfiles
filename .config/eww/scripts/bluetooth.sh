...
GetDevices () {
    ...
    do
        ...
        # If device is not connected, add eww button that connects on button click to the eww widget string.
        # If the device is connected, add eww button with different class (i.e. appearance) that disconnect 
        # on button click to the eww widget string
        if [[ "$connected_devices" != *"$device"* ]] 
        then
            eww_widget_string+="(button :class 'bluetoothPairedItemBtn' :onclick 'bluetoothctl connect $macAddress' '$device')"
        else
            eww_widget_string+="(button :class 'bluetoothConnectedItemBtn' :onclick 'bluetoothctl disconnect $macAddress' '$device')"
        fi
    done

    # Echo eww box containing all paired bluetooth devices
    echo "(box :class 'bluetoothMenuBox' :orientation 'vertical' :space-evenly true ${eww_widget_string})"
}
